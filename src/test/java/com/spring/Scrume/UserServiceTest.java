...
public class UserServiceTest extends AbstractTest {
	...
	public void userTestUpdate() throws Exception {
		Object[][] objects = { { super.entities().get("user2"), null, null },
				{ super.entities().get("user2"), "aCaCa123", ResponseStatusException.class },
				{ super.entities().get("user2"), "a1234", ResponseStatusException.class } };

		Stream.of(objects).forEach(x -> driverTestUpdate((Integer) x[0], (String) x[1], (Class<?>) x[2]));

	}

	protected void driverTestUpdate(Integer idUser, String password, Class<?> expected) {
		Class<?> caught = null;
		try {
			...
			if (password != null) {
				userDto.setPreviousPassword(userDB.getUserAccount().getPassword());
